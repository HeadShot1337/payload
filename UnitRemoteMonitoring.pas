...
begin
  ...
  if not Assigned(FFormatComboBox) then
  begin
    FFormatComboBox := TComboBox.Create(Self);
    FFormatComboBox.Parent := Panel1;
    FFormatComboBox.Left := ComboBox2.Left + ComboBox2.Width + 10;
    FFormatComboBox.Top := ComboBox2.Top;
    FFormatComboBox.Width := 90;
    FFormatComboBox.Style := csDropDownList;
    FFormatComboBox.Items.Add('JPEG');
    FFormatComboBox.Items.Add('H.264');
    FFormatComboBox.ItemIndex := 1; // Default to H.264
    FFormatComboBox.OnChange := FormatComboBoxChange;
  end;
...
begin
  ...
  if Assigned(FFormatComboBox) then
  begin
    if FFormatComboBox.ItemIndex = 0 then
      SelectedFormat := 1
    else
      SelectedFormat := 2;
  end;
...
procedure TForm6.FormatComboBoxChange(Sender: TObject);
begin
  if FCapturing then
