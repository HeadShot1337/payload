...
const
  ...
  MFVideoFormat_HEVC_GUID: TGUID = '{43564548-0000-0010-8000-00AA00389B71}'; { 'HEVC' }
...
begin
  ...
  { Create the H.265 (HEVC) decoder MFT }
  hr := CoCreateInstance(CLSID_CMSH265DecoderMFT, nil,
                          CLSCTX_INPROC_SERVER,
                          IMFTransform, Xfrm);
  if FAILED(hr) then Exit;

  { Input: H.265 compressed }
  GMFCreateMT(InType);
  InType.SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video_GUID);
...
begin
  ...
  { H.264 / H.265 decoders — created once, reused for all frames }
...
begin
  ...
  { Reset decoders so the next session starts clean }
...
begin
  ...
  { Update decoder hints — done outside the lock }
