...
type
  ...
  private
    ...
    FLineLocks        : TDictionary<TncLine, TCriticalSection>;
...
begin
  ...
  FLineLocks        := TDictionary<TncLine, TCriticalSection>.Create;
...
begin
  ...
  for var Lock in FLineLocks.Values do Lock.Free;
  FLineLocks.Free;
  FReadBuffers.Free;
...
begin
  ...
  try
    ...
    if ClientExists then
      FLineLocks.TryGetValue(aLine, LineLock);
  finally
    FLock.Leave;
  end;

  if ClientExists and Assigned(LineLock) then
  begin
      ...
      TThread.CreateAnonymousThread(
        ...
        begin
          ...
          try
            ...
            FLock.Enter;
            try
              Exists := FClients.ContainsKey(aLine);
            finally
              FLock.Leave;
            end;
...
begin
  ...
  try
    ...
    var OldLock: TCriticalSection;
    if FLineLocks.TryGetValue(aLine, OldLock) then OldLock.Free;
    FLineLocks.AddOrSetValue(aLine, TCriticalSection.Create);
  finally
    FLock.Leave;
  end;
...
begin
  ...
  try
    ...
    FInfoForms.Remove(aLine);
    var OldLock: TCriticalSection;
    if FLineLocks.TryGetValue(aLine, OldLock) then
    begin
      FLineLocks.Remove(aLine);
      OldLock.Free;
    end;
...
begin
  ...
  if (FrameHeader.Format < MONITOR_FRAME_FORMAT_JPEG) or
     (FrameHeader.Format > MONITOR_FRAME_FORMAT_H265) then Exit;
  ...
  if Assigned(MonitoringForm) then
    MonitoringForm.QueueFrameBytes(FrameBytes, Integer(FrameHeader.Format),
      Integer(FrameHeader.Width), Integer(FrameHeader.Height));
end;
